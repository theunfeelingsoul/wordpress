    ...
    switch ($type) {
        ...
        case 'wtf_fu_list_files' :
            return "<p>The <strong>[wtf_fu_list_files]</strong> shortcode is used to present a list of users uploaded files. The list format is configuarable to allow dispaly of filetype icons, thumnails, audio controls and download links<p/>
                ...
                <li>The format of each file row is determined by the <code>line_format</code> attribute. You may override this to change the display format. Any or all of the placeholders in the table below
                can be embedded in a table row html string. Any number of table cells &lt;td&gt;&lt;/td&gt;may be used but make sure you include the enclosing &lt;tr&gt; and &lt;/tr&gt; tags.</li>
                ...
                The default css can be found in the file .../work-the-flow-file-upload-pro/public/assets/css/wtf-fu-pro-list-files.css. The placeholders fields represent :"
                    . get_wtf_fu_list_files_placeholders_table()
                    . "</li>
...
function wtf_getFileInfo($filename, $paths) {
    ...
    $info->filetime = filemtime($filename);
...
function get_wtf_fu_list_files_placeholders_table() {

    $fields = array(
        "%%ICON%%" => "An icon representing the filetype, eg a PDF file or JPG image. Icons are provided in CSS from the flat-icon series provided free by Freepik, 
                the css files are available in the PRO package installation in the assets/css/flaticons directory.",
        "%%FILENAME%%" => "The filename of the file if the 'dowload_links' attribute of the wtf_fu_list_files shortcode is specifed and true (1) then the filename will also be a link to the file for download.",
        "%%FILESIZE%%" => "The filesize in human readable format e.g 2.52K",
        "%%FILEDATETIME%%" => "The file datetime in the format specified by the filetimeformat attribute.",
        "%%THUMBNAIL%%" => "The THUMBNAIL image that was created using the wtf_fu_upload shortcode if any, "
        . "to change the size do this in the wtf_fu_upload shortcode used to upload the image, "
        . "see thumbnail_width and thumbnail_height attributes",
        "%%MEDIUM%%" => "The MEDIUM image that was created using the wtf_fu_upload shortcode if any, "
        . "Medium images are not created by default to change this specify 'create_medium_images' in the wtf_fu_upload shortcode used to upload the image, "
        . "and also the medium_width and medium_height attributes."
        . "You may set this to any size (even if smaller than the thumbnails) and is a useful way to provide tiny thumbs for the list without affecting the thumnail images, even if the 'medium' name is misleading.",
        "%%NUMBER%%" => "Prints a number for the file in order starting from 1.",
        "%%AUDIO%%" => "Provides an embedded audio player for the file if it is an audio file.");

    $table = "<TABLE border=1><THEAD><TR><TH>%%FIELD%%</TH><TH>Meaning</TH></TR></THEAD><TBODY>";
