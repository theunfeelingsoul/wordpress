...
define('FILES_FM_UPLOAD__VERSION', '1.0');
...
function files_fm_upload__get_allowed_options () {
    return array('iframe_src', 'username', 'addhash', 'iframe_width', 'iframe_height');
}
...
function files_fm_upload__shortcode_handler( $atts ) {
    ...
    {
        $iframe = '<iframe src="'. $str_iframe_src . $config . $lang .'" frameborder="0" style="border: 0; width: '. (empty($str_iframe_width) ? '500px': $str_iframe_width) .'; height:'. (empty($str_iframe_height) ? '500px': $str_iframe_height) .'; overflow-x: hidden;"></iframe>';
    }
...
<p><?php echo $arr_settings['iframe_width'] ?>px</p>
<h3><?php _e( 'Upload box height' ); ?></h3>
<p><?php echo $arr_settings['iframe_height'] ?>px</p>
 -->
<?php endif;?>
...
<?php endif;?>
        ...
        jQuery('#files_fm_config_iframe').attr('src', '<?php echo esc_url( FILES_FM_UPLOAD__ADMIN_ORIGIN . FILES_FM_UPLOAD__ADMIN_PATH . '?lng=' . $str_lang_code . '' );?>' + str_user_data_param + (str_saved_username ? '&username='+str_saved_username : ''));
    }
